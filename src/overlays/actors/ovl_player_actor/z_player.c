...
void Player_ProcessAnimSfxList(Player* this, AnimSfxEntry* entry) {
    ...
    do {
        ...
        if (LinkAnimation_OnFrame(&this->skelAnime, fabsf(ANIMSFX_GET_FRAME(absData)))) {
            ...
            } else if (type == ANIMSFX_SHIFT_TYPE(ANIMSFX_TYPE_UNKNOWN)) { // only used in the intro cutscene
...
void func_80836BEC(Player* this, PlayState* play) {
    ...
    if ((play->csCtx.state != CS_STATE_IDLE) || (this->csAction != PLAYER_CSACTION_NONE) ||
        ...
        // In that case, setting `zTargetTimer` to 0 will stop Parallel if it is currently active.
        this->zTargetTimer = 0;
    } else if (this->zTargetTimer != 0) {
        this->zTargetTimer--;
    }

    if (this->zTargetTimer >= 6) {
        // When a lock-on is started, `zTargetTimer` will be set to 15 and then immediatley start decrementing
    ...
    if (isTalking || (this->zTargetTimer != 0) ||
