...
// Can't use OSContPad because gz excludes the error code field
typedef struct {
    u16 button;
    s8 stick_x;
    s8 stick_y;
...
typedef struct {
    CustomContPad contData;
    u16 buttonDelta;
} GzMacroFrame;

typedef struct {
    /* 0x00 */ size_t size; // size of macro; total number of input frames
    /* 0x04 */ size_t seed_size; // irrelevant for this feature
    ...
    // Some more fields here that aren't relevant...
} GzMacro;
